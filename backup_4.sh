#!/bin/bash
## ARQUIVO DE BACKUP DE SERVIDOR .4 DAQUI O SERVIDOR DE BACKUP TIRA AS INFORMACOES ##
#mount -t nfs 10.14.180.4:/media/backup /media/arquivos
arquivos="/media/arquivos"
backup=" /media/backup"
echo "Backup iniciado em $(date +%d"/"%b"/"%Y" as "%T)" >> /media/log/BACKUP_Servidor4.txt
...
echo -e "Backup concluido $(date +%d"/"%b"/"%Y" as "%T)\n" >> /media/log/BACKUP_Servidor4.txt
cp -f /media/log/BACKUP_Servidor4.txt /media/arquivos/programas/logs
